...
const Portfolio = () => {
  ...
  const fetchRepositories = async () => {
    const response = await fetch('https://api.github.com/users/farzadsnj/repos');
          ...
          <div className="grid grid-cols-2 gap-2">
            {filteredRepositories.length > 0 ? (
              filteredRepositories.map(repo => (
                <div key={repo.id} onClick={() => window.open(repo.html_url, '_blank')} className="border p-4 rounded-md cursor-pointer hover:text-black hover:bg-gray-100 transition-colors">
                  <h2>{repo.name}</h2>
                </div>
              ))
            ) : (
